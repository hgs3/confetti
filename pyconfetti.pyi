# Confetti: a configuration language and parser library
# Copyright (c) 2025 Confetti Contributors
#
# This file is part of Confetti, distributed under the MIT License
# For full terms see the included LICENSE file.
# <https://RailgunLabs.com/license>.

from typing import Set, Iterator
from collections.abc import Sized, Iterator

class Confetti(Sized, Iterator[Directive]):
    def __init__(self, source: str, c_style_comments: bool = False, expression_arguments: bool = False, punctuator_arguments: Set[str] = False) -> None: ...
    # Iteration.
    def __len__(self) -> int: ...
    def __iter__(self) -> DirectiveIterator: ...
    def __next__(self) -> Directive: ...
    @property
    def comments(self) -> CommentIterator: ...

class CommentIterator(Sized, Iterator[Comment]):
    def __len__(self) -> int: ...
    def __next__(self) -> Comment: ...

class Comment:
    @property
    def text(self) -> str: ...

class Directive(Sized):
    def __len__(self) -> int: ...
    def __iter__(self) -> DirectiveIterator: ...
    @property
    def args(self) -> ArgumentIterator: ...

class DirectiveIterator(Sized, Iterator[Directive]):
    def __len__(self) -> int: ...
    def __next__(self) -> Directive: ...

class Argument:
    @property
    def value(self) -> str: ...
    @property
    def expression(self) -> bool: ...

class ArgumentIterator(Sized, Iterator[Argument]):
    def __len__(self) -> int: ...
    def __iter__(self) -> ArgumentIterator: ...
    def __next__(self) -> Argument: ...

class IllegalSyntaxError(SyntaxError):
    pass
