...
if command -v gcc &> /dev/null; then
    CC=gcc cmake .. -B ${OUTDIR} -G "Ninja" -DCONFETTI_CODE_COVERAGE=ON
    cmake --build ${OUTDIR}
    cmake --build ${OUTDIR}
...
if command -v clang &> /dev/null; then
    ...
    CC=clang cmake .. -B ${OUTDIR} -G "Ninja" -DCONFETTI_UNDEFINED_BEHAVIOR_SANITIZER=ON
    cmake --build ${OUTDIR}
    ctest --test-dir ${OUTDIR} --parallel --output-on-failure
    cmake -E remove_directory ${OUTDIR}

    # Run the Address Sanitizer.
    CC=clang cmake .. -B ${OUTDIR} -G "Ninja" -DCONFETTI_ADDRESS_SANITIZER=ON
    cmake --build ${OUTDIR}
    ctest --test-dir ${OUTDIR} --parallel --output-on-failure
    cmake -E remove_directory ${OUTDIR}

    # Run the Memory Sanitizer.
    CC=clang cmake .. -B ${OUTDIR} -G "Ninja" -DCONFETTI_MEMORY_SANITIZER=ON
    cmake --build ${OUTDIR}
    ctest --test-dir ${OUTDIR} --parallel --output-on-failure
    cmake -E remove_directory ${OUTDIR}
else
